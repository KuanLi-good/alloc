...
void init_allocator(void * heapstart, uint8_t initial_size, uint8_t min_size) {
    ...
    // printf("min_size: %d\n",min_size);
    virtual_sbrk(sizeof(VIRTUAL_HEAP)+ (pow(2, (initial_size-min_size))*2-1)*sizeof(uint32_t) + pow(2, initial_size));
    ((VIRTUAL_HEAP*)heapstart)->heap_length = sizeof(VIRTUAL_HEAP)+ (pow(2, (initial_size-min_size))*2-1)*sizeof(uint32_t) + pow(2, initial_size);
    ((VIRTUAL_HEAP*)heapstart)->data_start = sizeof(VIRTUAL_HEAP) + (pow(2, (initial_size-min_size))*2-1)*sizeof(uint32_t);
...
// 0x7ff9a651002c
    ...
    for(int i = 0; i < ((VIRTUAL_HEAP*)heapstart)->tree_length; i++){
        ...
        ((VIRTUAL_HEAP*)heapstart)->tree_1D[i] = pow(2, initial_size);
...
void * virtual_malloc(void * heapstart, uint32_t size) {
    ...
    ((VIRTUAL_HEAP*)heapstart)->node_size = pow(2, ((VIRTUAL_HEAP*)heapstart)->initial_size);
    ...
    // printf("ceil_size: %d\n", size);
    if(size > ((VIRTUAL_HEAP*)heapstart)->tree_1D[0]) {
        // puts("size > tree[0]");
        return NULL;
    }
    ...
    while(((VIRTUAL_HEAP*)heapstart)->node_size > size) {
       ...
       // if both leaves have available space, choose the smaller one
        ...
        // if only one leaf has enough space
            ...
            ((VIRTUAL_HEAP*)heapstart)->index = right_leaf(((VIRTUAL_HEAP*)heapstart)->index);
        ((VIRTUAL_HEAP*)heapstart)->node_size /= 2;               
    }

    // beginning of buddy allocator block + offset
    // ( offset = (index + 1) * block_size - total_size)
    ((VIRTUAL_HEAP*)heapstart)->offset = (int)((VIRTUAL_HEAP*)heapstart)->node_size * (((VIRTUAL_HEAP*)heapstart)->index + 1) - (int)pow(2, ((VIRTUAL_HEAP*)heapstart)->initial_size);

    reassign_value(heapstart, ((VIRTUAL_HEAP*)heapstart)->index);
    // virtual_sbrk(-(int32_t)(2*sizeof(uint32_t)+sizeof(void*)));
    // printf("ptr: %p\n", ((VIRTUAL_HEAP*)heapstart)->ptr);
...
int find_index(void* heapstart, void* ptr) {
    ...
    ((VIRTUAL_HEAP*)heapstart)->node_size = pow(2, ((VIRTUAL_HEAP*)heapstart)->min_size);
    // printf("node_size: %d\n", ((VIRTUAL_HEAP*)heapstart)->node_size);

    // index = offset... here we assure the smallest block
    // ( offset = (index + 1) * block_size - total_size)
    ((VIRTUAL_HEAP*)heapstart)->index = ((VIRTUAL_HEAP*)heapstart)->offset / ((VIRTUAL_HEAP*)heapstart)->node_size + (((VIRTUAL_HEAP*)heapstart)->tree_length + 1)/2 - 1;
    // printf("index: %d\n", ((VIRTUAL_HEAP*)heapstart)->index);
    // printf("index holds: %d\n", ((VIRTUAL_HEAP*)heapstart)->tree_1D[((VIRTUAL_HEAP*)heapstart)->index]);
...
void merge_block(void* heapstart) {
    while (((VIRTUAL_HEAP*)heapstart)->index) {
        ...
        if(((VIRTUAL_HEAP*)heapstart)->tree_1D[left_leaf(((VIRTUAL_HEAP*)heapstart)->index)] + ((VIRTUAL_HEAP*)heapstart)->tree_1D[right_leaf(((VIRTUAL_HEAP*)heapstart)->index)] == ((VIRTUAL_HEAP*)heapstart)->node_size)
                ((VIRTUAL_HEAP*)heapstart)->tree_1D[((VIRTUAL_HEAP*)heapstart)->index] = ((VIRTUAL_HEAP*)heapstart)->node_size;
        else {
...
int virtual_free(void * heapstart, void * ptr) {
    // printf("index: %d\n", 0);
    // printf("index holds: %d\n", ((VIRTUAL_HEAP*)heapstart)->tree_1D[0]);
    //  for(int i = 0; i < ((VIRTUAL_HEAP*)heapstart)->tree_length; i++){
    //     if (power_of_2(i+1)){
    //         puts("");
    //     }   
...
void * virtual_realloc(void * heapstart, void * ptr, uint32_t size) {
    ...
    if(!find_index(heapstart, ptr)) {
        ...
        virtual_sbrk(sizeof(uint32_t)*2 + sizeof(uint64_t));
        uint32_t *node_size = heapstart + ((VIRTUAL_HEAP*)heapstart)->heap_length;
        uint32_t *index = heapstart + ((VIRTUAL_HEAP*)heapstart)->heap_length + sizeof(uint32_t);
        uint64_t *offset = heapstart + ((VIRTUAL_HEAP*)heapstart)->heap_length + 2*sizeof(uint32_t);
...
void offset_traversal(void *heapstart, uint32_t index, uint32_t node_size) {
    // printf("index: %d\n", index);
    node_size = find_node_size(node_size, index, ((VIRTUAL_HEAP*)heapstart)->initial_size);
    ...
    }else if(index && ((VIRTUAL_HEAP*)heapstart)->tree_1D[index] ==  find_node_size(node_size, index, ((VIRTUAL_HEAP*)heapstart)->initial_size)){
        if (((VIRTUAL_HEAP*)heapstart)->tree_1D[parent(index)]){ 
            // node on the left
            if(index%2 && ((VIRTUAL_HEAP*)heapstart)->tree_1D[index+1] !=  find_node_size(node_size, index+1, ((VIRTUAL_HEAP*)heapstart)->initial_size))
                printf("free %d\n", find_node_size(node_size, index, ((VIRTUAL_HEAP*)heapstart)->initial_size));
            // node on the right
            else if (!(index%2) && ((VIRTUAL_HEAP*)heapstart)->tree_1D[index-1] !=  find_node_size(node_size, index-1, ((VIRTUAL_HEAP*)heapstart)->initial_size))
                printf("free %d\n", find_node_size(node_size, index, ((VIRTUAL_HEAP*)heapstart)->initial_size));
        }
    }
    {
...
void virtual_info(void * heapstart) {
    ...
    // }
    // puts("");
    if(((VIRTUAL_HEAP*)heapstart)->tree_1D[0] == pow(2, ((VIRTUAL_HEAP*)heapstart)->initial_size)) {
